...
import com.provizit.kioskcheckin.activities.Meetings.MeetingCheckInDoneActivity;
            ...
            if (spre[0].trim().equalsIgnoreCase("meeting")) {
                ...
                if (spre.length == 3) {
                    ...
                    if (str.equalsIgnoreCase("+")) {
                        ...
                        MeetingScannedMobileMethod(last24Chars);
                    } else {
                        newVal = newVal.trim().replaceAll("\u0000", "");
                        visitor_email.setText(newVal.trim());
                        MeetingScannedEmailMethod(newVal.trim(), last24Chars);
                    }
                } else {
                    visitor_mobile.getText().clear();
                                    ...
                                    try {
                                        if (model.getResult() == 200) {
                                            ...
                                            if (Qrmodel.getItems().getCheckin() != (0) && Qrmodel.getItems().getCheckout() == (0)) {
                                                ...
                                                intent1.putExtra("getQrCodeStatus_Badge", Qrmodel.getItems().getBadge());
                                                intent1.putExtra("getQrCodeStatus_DateTime", Qrmodel.getItems().getDatetime().get$numberLong());
...
//                        }
    ...
    private void MeetingScannedMobileMethod(String last24Chars) {
...
//            visitor_mobile.requestFocus();
                ...
                try {
                    if (model.getResult() == 200) {
                        ...
                        apiViewModel.getQrcodeStatus_response().observe(this, QRmodel -> {
                            try {
                                if (QRmodel.getResult() == 200) {
                                    if (QRmodel.getItems().isMcheckin_status()) {
                                        //once meeting checkin done after that move to this screen whitOut OTP
                                        Intent intent1 = new Intent(getApplicationContext(), MeetingCheckInDoneActivity.class);
                                        startActivity(intent1);
                                    } else {
                                        //meeting no checkIn redirect to OTP
                                        Intent intent = new Intent(getApplicationContext(), OTPActivity.class);
                                        intent.putExtra("model_key", model);
                                        startActivity(intent);
                                    }
    ...
    private void MeetingScannedEmailMethod(String decodeResult, String last24Chars) {
        Preferences.saveStringValue(VisitorLoginActivity.this, Preferences.country_Code, ccp.getSelectedCountryCode());
